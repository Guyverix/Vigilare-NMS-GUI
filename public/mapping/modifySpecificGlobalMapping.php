      ...
      <div class="container">
        ...
        <div class='container-fluid mt-2'>
          <ul id="bc1" class="breadcrumb"><span id="ellipses" style="display: inline;">...</span>
            ...
            <li><a href="/mapping/modifyTrapMapping.php">Modify Poller Mapping</a></span> </li>
          </ul>
        </div>
        ...
        $outputArray = json_decode($outputRaw, true);
        curl_close($ch);

        /* We are only getting one return back */
        foreach ($outputArray as $output1) {
          foreach ($output1 as $output) {
          $checkName      = $output['checkName'];
          $checkAction    = $output['checkAction'];
          $iteration      = $output['iteration'];
          $storage        = $output['storage'];
          $hostname       = $output['hostname'];
          $type           = $output['type'];
          $hostGroup      = $output['hostGroup'];
          $id             = $output['id'];
          }
        }
        ?>

        <form id="form" action="<?php echo $apiUrl . ':' . $apiPort; ?>/globalMapping/poller/update"  method="POST" onsubmit="return submitForm(this);">
          <input type="hidden" name="old_checkName" value="<?php echo $output['checkName'] ?>">
          <input type="hidden" name="old_checkAction" value="<?php echo $output['checkAction'] ?>">
          <input type="hidden" name="old_iteration" value="<?php echo $output['iteration'] ?>">
          <input type="hidden" name="old_type" value="<?php echo $output['type'] ?>">
          <input type="hidden" name="old_storage" value="<?php echo $output['storage'] ?>">
          <input type="hidden" name="id" value="<?php echo $id ?>">
          <div class="mb-3 md-3">
            <button type="button" class="btn btn-sm btn-primary" title="Alarm Name" data-bs-toggle="popover" data-bs-content="Alarm names must never contain spaces.  This will cause heart burn in other areas.  Recommend camelCase and dashes to make good alarm names">?</button>
            <label for="checkName" class="form-label">Alarm Name:</label>
            <input type="displayName" class="form-control" id="checkName" value="<?php echo $checkName ; ?>" name="checkName">
          </div>
          <div class="mb-3">
            <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="popover" title="Command Action" data-bs-content="Define the command or OID value to check">?</button>
            <label for="displayName" class="form-label">Command to run is of type:  SNMP OID / NRPE command / Shell Command / Curl (future):</label>
            <input type="displayName" class="form-control" id="checkAction" value="<?php echo $checkAction; ?>" name="checkAction">
          </div>
          <div class="mb-3">
            <button type="button" class="btn btn-sm btn-primary" title="Iteration Cycle" data-bs-toggle="popover" data-bs-content="Run command every X number of seconds">?</button>
            <label for="iteration" class="form-label">Iteration:</label>
            <input type="displayName" class="form-control" id="iteration" value="<?php echo $iteration; ?>" name="iteration">
          </div>
          <div class="mb-3">
            <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="popover" title="Poller Types" data-bs-content="shell, (SNMP: walk, get), nrpe, curl ">?</button>
            <label for="type" class="form-label">Poller Type:</label>
            <input type="displayName" class="form-control" id="type" value="<?php echo $type; ?>" name="type" >
          </div>
          <div class="mb-3">
            <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="popover" title="Storage" data-bs-content="String with ONE value: database, graphite, databaseMetric">?</button>
            <label for="storage" class="form-label">Metric Storage Type:</label>
            <input type="displayName" class="form-control" id="storage" value="<?php echo $storage ; ?>" name="storage">
          </div>
          <div class="mb-3">
            <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="popover" title="Hostgroup changes" data-bs-content="Add or remove hostgroups in CSV format">?</button>
            <label for="hostGroup">Hostgroup names as csv:</label>
            <textarea class="form-control" rows="2" id="hostGroup" name="hostGroup"> <?php echo $hostGroup ; ?></textarea>
          </div>
          <div class="mb-3">
            <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="popover" title="Hostname changes" data-bs-content="Add or remove hostnames in CSV format">?</button>
            <label for="hostname" class="form-label">Hostname names as csv:</label>
            <textarea class="form-control" rows="2" id="hostname" name="hostname"> <?php echo $hostname; ?></textarea>
          </div>
          <button type="submit" class="btn btn-danger" disabled>Test Changes</button>
          <button type="submit" class="btn btn-primary"  value="post request" form="form">Update Mapping</button>
        </form>
      </div>
    </main>
  </div>
