        ...
        <main>
          <div class="container-fluid">
            <div class='container mt-2'>
              ...
              <li><a href="/graph/hostTableAll.php?id=<?php echo $rawHostname; ?>">All Graphs</a><span class="divider"> <span class="accesshide "><span class="arrow_text">/</span> </span><span class="arrow sep">►&nbsp</span> </span>
              </li>
              <li><span tabindex="0">&nbsp<?php echo $rawTablename; ?>&nbsp</span></li>
              </ul>
            </div>
...
curl_setopt($ch, CURLOPT_POSTFIELDS, "job=single&hostname=$rawHostname");
// Execute
...
if ( preg_match( '/unable to retrieve metric/' ,$result['data'][0])) {
  ...
  foreach ($result as $lists) {
    foreach($lists as $list) {
      ...
      if ( strpos($list, ".$rawTablename") != false) {
      switch ($list) {
...
//  $st2='<img id="image-viewer" src=';
  ...
  if ( ! empty($cleanInterfaces)) {
    ...
    foreach ($cleanInterfaces as $key => $value) {
      ...
      foreach ($value as $k => $v) {
        ...
        $vc=preg_replace('/308/','100', $vc);
        $v=preg_replace('/586/', '1200', $v);
        $v=preg_replace('/308/', '300', $v);
        // counterFoo
        if ( $counter == 0 ) {
  ...
  if ( ! empty ($cleanDefault)) {
    foreach ($cleanDefault as $key => $value) {
      ...
      $counter=0;
      foreach ($value as $k => $v) {
        $v=preg_replace('/586/', '250', $v);
        $v=preg_replace('/308/','150', $v);
        // counterFoo
        if ( $counter == 0 ) {
         echo "<tr><td><b><center>" . $k ."</center></b><br>" . $st1 . $v . $st3 . $v . $ed1;
         $counter++;
        }
        elseif ( $counter <=3 ) {
          echo "<td><b><center>" . $k ."</center></b><BR>". $st1 . $v . $st3 . $v . $ed1;
          $counter++;
        }
        elseif( $counter >= 4 ) {
          echo "<td><b><center>" . $k ."</center></b><BR>" . $st1 . $v . $st3 . $v . $ed1 . "</tr>";
          $counter=0;
        }
      } // end foreach
      $counter=0;
