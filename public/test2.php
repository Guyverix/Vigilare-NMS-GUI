<?php
/**
 * Vigilare — Landing Dashboard (Bootstrap 5, minimal JS)
 * Single-file example you can drop into your GUI and wire up to your data.
 *
 * Replace the sample $data loaders with calls to your existing API helpers
 * (e.g., callApiPost()) and map into the arrays used below.
 */
...
$summary = [
  ...
  'mtta'    => '12m', // example: Mean Time To Acknowledge
  'mttr'    => '42m', // example: Mean Time To Restore
];

$alerts = [
  // id, time, device, severity, summary, acked
...
function sevBadge(int $sev): string {
  // Map to Bootstrap background classes
  $cls = match(true) {
    $sev >= 5 => 'bg-danger',
...
<style>
body { background-color: #0f1115; color: #f5f5f5; }
.card { background-color: #151923; border: 1px solid #232838; }
.card-header { background: linear-gradient(180deg,#1b2030,#121622); border-bottom: 1px solid #232838; }
.stat-lg { font-size: 1.8rem; font-weight: 700; color: #ffffff; }
.muted { color:#c2c6d3; }
a, a:hover { color: #a4d0ff; }
.table thead th { color:#d0d6e8; border-bottom-color:#2a3042; }
.table tbody tr { border-color:#222838; }
.table tbody td { color: #f9f9f9; }
    ...
    <div class="col-xl-3 col-md-6">
      <div class="card h-100">
        ...
        <div class="card-body">
          ...
          <div class="mt-3 small text-secondary">Based on devices down/unreachable.</div>
        </div>
      </div>
    </div>
      ...
      <div class="card h-100">
        ...
        <div class="card-body">
          <div class="row g-2 text-center">
            ...
            <div class="col-3"><div class="muted">Unk</div><div class="stat-lg text-secondary"><?php echo (int)$summary['checks']['unknown']; ?></div></div>
          </div>
          <div class="mt-3">
            <?php
              // Stacked indicator using simple ratios
            ...
            <div class="small mt-2 text-secondary">Critical/Warning/OK/Unknown mix</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="col-xl-7">
      <div class="card h-100">
        <div class="card-header text-white d-flex justify-content-between align-items-center">
          ...
          <a class="small" href="/events/index.php">All events →</a>
        </div>
        <div class="card-body p-0">
          <div class="table-responsive">
            <table class="table table-hover table-sm align-middle mb-0">
              ...
              <tbody>
              <?php if (empty($alerts)): ?>
                <tr><td colspan="5" class="text-center text-secondary py-4">No active alerts 🎉</td></tr>
      ...
      <div class="card h-100">
        ...
        <div class="card-body">
          <?php if (empty($topResources)): ?>
            <div class="text-secondary">No high utilization right now.</div>
          ...
          <div class="row g-2">
            ...
            ?>
              <div class="col-6 col-md-4">
                <div class="p-2 rounded border" style="border-color:#232838">
                  <div class="d-flex justify-content-between align-items-center mb-1">
                    <div class="fw-semibold"><?php echo safe($s['site']); ?></div>
                    <span class="badge <?php echo $stateCls; ?> text-uppercase"><?php echo safe($s['status']); ?></span>
                  </div>
                  <div class="small text-secondary">Up: <?php echo (int)$s['up']; ?> · Down: <span class="text-danger"><?php echo (int)$s['down']; ?></span></div>
                </div>
              </div>
            <?php endforeach; ?>
          </div>
      ...
      <div class="card h-100">
        ...
        <div class="card-body">
          <?php if (empty($maintenance)): ?>
            <div class="text-secondary">No scheduled maintenance.</div>
          <?php else: ?>
            <ul class="list-unstyled mb-0">
              <?php foreach ($maintenance as $m): ?>
              <li class="mb-2">
                <div class="fw-semibold"><?php echo safe($m['when']); ?></div>
                <div class="small">Scope: <a href="/host/view.php?name=<?php echo urlencode($m['scope']); ?>"><?php echo safe($m['scope']); ?></a></div>
                <div class="small text-secondary"><?php echo safe($m['note']); ?></div>
              </li>
              <?php endforeach; ?>
            </ul>
          <?php endif; ?>
      ...
      <div class="card h-100">
        ...
        <div class="card-body">
          <?php if (empty($tickets)): ?>
            <div class="text-secondary">No active tickets.</div>
          <?php else: ?>
            <ul class="list-group list-group-flush">
              <?php foreach ($tickets as $t): ?>
              <li class="list-group-item" style="background:#151923; border-color:#232838;">
                <div class="d-flex justify-content-between">
                  <a href="/tickets/view.php?id=<?php echo urlencode($t['id']); ?>" class="fw-semibold"><?php echo safe($t['id']); ?></a>
                  <span class="badge <?php echo $t['sev']==='P1'?'bg-danger':($t['sev']==='P2'?'bg-warning text-dark':'bg-secondary'); ?>"><?php echo safe($t['sev']); ?></span>
                </div>
                <div class="small"><?php echo safe($t['title']); ?></div>
                <div class="small text-secondary">Status: <?php echo safe($t['status']); ?></div>
              </li>
              <?php endforeach; ?>
            </ul>
          <?php endif; ?>
