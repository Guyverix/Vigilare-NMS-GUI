...
<head>
  ...
  <title>Daemon Control and Status</title>
              ...
              <tfoot>
                ...
                <?php
                  ...
                  */
                  ...
                  // These will never have start stop and are not valid monitoring daemons but CAN ( not do )  have a heartbeat
                  $ignoreType = ["mysql", "snmptrapd", "heartbeat", "passive"];

                  // Unlikely, but someone may disable a daemon but still allow it to exist.  These are permitted names
                  $ignoreDisabled = ["ignore", "disable", "disabled", "bypass", "bypassed"];
                  ...
                  foreach ($configuredDaemons['data'] as $confed) {
                    foreach ($confed as $confKey => $confValue) {
                      foreach($heartbeat['data'] as $aDaemon) {
                        // confKey is pollerList name
                        // aDaemon['device'] is pollerFull name

                        $aDaemon['device'] = trim($aDaemon['device']);
                        $confKey = trim($confKey);
                        ...
                        if( strpos(strtolower($aDaemon['device']), $confKey) !== false && strpos($aDaemon['component'], $confValue) !== false ) {
                          ...
                          if ($confKey == 'ping' && $aDaemon['device'] == 'housekeeping' ) { // do nothing, as they are not a real match
                            ...
                            else { $tableData .= "<center>Controlled by Operating System</center></tr>";
                            }  // end not snmptrapd
                ...
                foreach ( $resultSet as $deadKey => $deadValue) {
                  // echo "deadKey " . $deadKey . " deadValue " . $deadValue . "<br>";
                  if ( ! in_array($deadKey, $ignoreDisabled)) {
